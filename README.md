...
3. **(Optional) Set the `VITE_API_URL` repo variable** (Settings → Actions → Variables) — it's baked into the web image's client bundle at build time (Vite's `import.meta.env`, not a runtime env read), so it must be whatever URL browsers should reach the server at. Defaults to `http://localhost:3000` if unset, which only works when the browser and the published image are on the same machine.
